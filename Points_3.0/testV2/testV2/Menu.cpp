...
void Menu::setNext(int next) {
	nextState_ = next;
}

void Menu::menuProces(sf::RenderWindow & window, CBodyBasics & skelet) {
	...
	while (isMenu_)
	{
		...
		{
			if (menuNum == 1) { isMenu_ = false; nextState_ = 2; } //если нажали первую кнопку, то выходим из меню 
			if (menuNum == 2) { window.draw(about); window.display(); while (!Keyboard::isKeyPressed(Keyboard::Escape)); }
			if (menuNum == 3) { window.close(); isMenu_ = false; }
